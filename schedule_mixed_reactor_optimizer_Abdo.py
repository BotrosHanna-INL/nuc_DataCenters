...
def min_tot_P(power_list , initial_delay_list, levelization_period_weeks):
    """
...
def initial_population(power_list, sol_per_pop):
    ...
    if fuel_cycle_length(max(power_list)) == fuel_cycle_length(min(power_list) ):
        ...
        if int(fuel_lifetime_to_refueling_ratio)   >= num_reactors:
            ...
            initial_population = np.zeros((sol_per_pop,num_intervals),dtype=np.int64) #
            for i in range(sol_per_pop) :
                initial_population_init = np.random.randint(0, the_max_delay+1, size=num_reactors)
                initial_population_0 = (np.hstack([initial_population_init]* int(np.ceil(num_reactors/len(initial_population_init)))))[:num_reactors]
                initial_population[i,:] = initial_population_0.astype(np.int64)
            ...
            num_intervals = int(np.ceil(the_max_delay/refuel_period)) +1
            initial_population = np.zeros((sol_per_pop,num_reactors),dtype=np.int64) #
            for i in range(sol_per_pop ) :
                initial_population_init = np.random.randint(0, the_max_delay+1, size=num_reactors) #np.random.rand(0, the_max_delay, num_reactors)
                initial_population_0 = (np.hstack([initial_population_init]* int(np.ceil(num_reactors/len(initial_population_init)))))[:num_reactors]
                initial_population[i,:] = initial_population_0.astype(np.int64)
    ...
    # if they don't have the same category (mix of smr, micro, large reactors)
        ...
        for i in range(sol_per_pop ) :
            initial_population_init = np.random.randint(0, the_max_delay+1, size=num_reactors)
            initial_population_0 = (np.hstack([initial_population_init]* int(np.ceil(num_reactors/len(initial_population_init)))))[:num_reactors]
            initial_population[i,:] = initial_population_0

    return initial_population.tolist(), allow_duplicate, expected_output


def on_gen(ga_instance):
    """
    This function is a callback function that is called by the Genetic Algorithm (GA) library at each generation.
    It is used to print the current generation number, the best solution's fitness, and the best solution itself.

    Parameters:
    ga_instance (pygad.GA): The instance of the Genetic Algorithm library.

    Returns:
    None. This function does not return any value. It is used for printing the current generation number, the best solution's fitness, and the best solution itself.
    """
...
def optimize_schedule(power_list,  levelization_period_weeks ): # Note that the power list here includes all the values of power (with repetition)
    """
        This function optimizes the schedule of reactors to minimize the total power output over a given time period,
        considering the initial delay, fuel cycle length, refueling duration, and power of each reactor.
        ...
        Returns:
        - schedule_times (list of int): A list of time (in weeks) after all the reactors are in service.
        - schedule_powers (list of list of float): A list of lists, where each inner list represents the power output of each reactor at a specific time.
        - schedule_rampup_duration (int): The maximum initial delay (in weeks) among all the reactors.
        - sol (list of float): The best solution found by the Genetic Algorithm (GA) library.

        The function first calculates the fuel lifetime-to-refueling ratio for the reactors with the highest and lowest power levels.
        If this ratio is greater than or equal to the number of reactors, the function assumes that all reactors have the same category (e.g. SMR) and calculates an initial guess for the population based on this assumption.
        If the ratio is less than the number of reactors, the function assumes that the reactors have different categories (mix of SMR, micro, large reactors) and returns an error.

        The function then returns the initial population, allows for duplicate solutions, and expects an output discrepancy of 0.
        It uses the Genetic Algorithm (GA) library to find the best solution, and the on_gen function is called by the GA library at each generation.

        The function calculates the fitness of each solution based on the discrepancy between the expected output and the actual output.
        It also calculates the capacity factor for each reactor and the overall capacity factor for all reactors.

        The function then extracts the time and power after excluding the initial ramp-up period, and calculates the nominal and actual power for each day.
        It also calculates the overall capacity factor for all reactors over the entire time period.

        Finally, the function calculates the time in years, the capacity factor for each reactor and the overall capacity factor for all reactors, and the excess energy produced by each reactor.

        The function returns the schedule times, schedule powers, schedule ramp-up duration, and the best solution found by the GA library.
    """
    start_time = time.time()
    # print('*********Starting Inner*************')
    # GA params
    sol_per_pop = 100 # int(2*len(power_list))



    num_generations = 300
    num_parents_mating =  int(sol_per_pop /2) # 2
    num_genes = len(power_list)
    init_range_low = 0
    init_range_high = 2

    parent_selection_type = "rank"
    keep_parents =  int(sol_per_pop / 4) # 2
    gene_type= int
    ...
    initial_pop,allow_dup,expected_out  = (initial_population(power_list, sol_per_pop))
    ...
    np.savetxt('fitness_inner.csv', np.array(fits), delimiter=',')
    # plt.plot(fits)
    # plt.show()
...
def capacity_factor_weeks_approach_mix_reactors(long_list_of_powers,levelization_period_weeks, demand):
    ...
    Parameters:
    - long_list_of_powers (list of float): A list of the power of each reactor.
    - levelization_period_weeks (int): The total time period (in weeks) over which the calculation is performed.
    - demand (float): The total power demand per week.

    Returns:
    - times (list of int): A list of time (in weeks) after all the reactors are in service.
    - capacity_factor_t (list of float): A list of capacity factors for each time step.
    - overall_capacity_factor (float): The overall capacity factor for all reactors over the entire time period.
    - MW_hours_generated_per_year_total (list of float): A list of total MW hours generated per year.
    - MW_hours_generated_per_year_per_demand (list of float): A list of MW hours generated per year per demand.
    - MW_hours_excess_per_year (list of float): A list of excess MW hours generated per year.
    - yearly_OM_cost_list (list of float): A list of yearly OM costs.
    ...
    The function returns the schedule times, schedule powers, schedule ramp-up duration, and the best solution found by GA library.
    """
